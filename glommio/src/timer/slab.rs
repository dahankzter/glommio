...
impl Generation {
    ...
    /// Wrapping is unreachable: at a billion reuses a second this takes 584
    /// years. Skipping zero keeps `NonZeroU64`'s niche so `Option<Generation>`
    /// stays eight bytes.
    fn next(self) -> Self {
        let raw = self.0.get().wrapping_add(1).max(1);
        Generation(NonZeroU64::new(raw).expect("kept non-zero above"))
    }
}
